...
for pagamentosformas, formas_de_pagamento in enumerate(formas_de_pagamento, start=1):
    aaa = f"{pagamentosformas}º Forma de pagamento: {formas_de_pagamento}"
    print(aaa)
...
x_inicial = 10
y_inicial = 117
# Define o espaço vertical entre os itens
espaco_entre_linhas = 6  # Ajuste conforme necessário para a altura dos itens
# Define a cor do texto como preto
pdf.set_text_color(0, 0, 0)

# Variável de controle da posição vertical
y = y_inicial

# Percorre todos os itens da lista e adiciona um abaixo do outro
for item in formas_de_pagamento:
    pdf.text(x_inicial, y, item)
    y += espaco_entre_linhas  # Move para a próxima linha
