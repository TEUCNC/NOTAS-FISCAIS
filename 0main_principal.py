...
def verif_num_pode_vazio(prompt):
    while True:
        ...
        try:
            entrada_num = float(entrada)  # Tenta converter a entrada para número inteiro
            return entrada_num  # Retorna o valor se for válido
        except ValueError:
...
def pedido():
    ...
    for ii in range(itens_venvidos):
        item_ordem = 1 + ii
        item_class_cit = int(input(f"Qual é a classificação do {item_ordem}º produto? \n"
                                   f"1) Notebook\n"
                                   f"2) Monitor\n"
                                   f"3) Escritório\n"
...
itens_venvidos = int(input("Foram vendidos quantos itens para o cliente? "))
...
mes_atual = datetime.now().strftime("%B").lower()  # Nome do mês atual em minúsculas

# Gera o nome completo do arquivo
numero_sequencial = f"{proximo_numero_arquivo(folder_path)}_{mes_atual}"
...
imagem_largura = 65  # Largura da imagem em mm
imagem_altura = 55  # Altura da imagem em mm
...
imagem_largura = 50  # Largura da imagem em mm
imagem_altura = 50  # Altura da imagem em mm
# Obtém as dimensões da página (210 x 297 para A4)
largura_pagina = pdf.w  # Largura da página A4
altura_pagina = pdf.h  # Altura da página A4
# Calcula as coordenadas x e y para posicionar a imagem no canto inferior direito
...
# Define a fonte: Times, tamanho 16
...
soma_precos_str = str(soma_precos)
juros_str = str(juros)
itens.append("Juros R$" + juros_str)
itens.append("TOTAL R$" + soma_precos_str)
...
if combinar_pagamento in ("S", "SIM"):
    ...
    while quant_pagamentos not in (1, 2, 3):
        print("Oferecemos apenas 3 combinações de pagamento!!")
        sleep(2.5)
        limpartela()
        quant_pagamentos = verif_num("Serão quantas formas de pagamento? ")
